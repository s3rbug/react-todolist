...
export class GoalsService {
	...
	async getGoalById(user: User, getGoalDto: GetGoalDto) {
		const userFound = await this.userModel.findOne({ username: user.username })
		const folderFound = userFound.folders.find(
			(folder) => folder.id === getGoalDto.folderId
		)
		if (!folderFound) {
			throw new HttpException("Wrong folder id", HttpStatus.BAD_REQUEST)
		}
		const goalFound = folderFound.goals.find(
			(goal) => goal.id === getGoalDto.goalId
		)
		if (!goalFound) {
	...
	async swapGoalsDifferentFolders(
		user: User,
		swapData: SwapGoalDifferentFolders
	) {
		const userFound = await this.userModel.findOne({ username: user.username })
		const { fromFolderId, toFolderId, fromGoalIndex, toGoalIndex } = swapData
		const goalsToMove = userFound.folders.find(
