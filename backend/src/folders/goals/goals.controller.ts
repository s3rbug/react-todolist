import { SwapGoalsSameFolderDto, SwapGoalDifferentFolders } from './dto/swap-goals.dto';
...
export class GoalsController {
    ...
    @Get(":folderId")
    async getAllGoals(@Req() req: AuthRequest, @Param('folderId') folderId: string){
    ...
    async swapGoalsSameFolder(@Req() req: AuthRequest, @Body() swapData: SwapGoalsSameFolderDto){
        return this.goalsService.swapGoalsSameFolder(req.user, swapData)
    }

    @UseGuards(JwtAuthGuard)
    @Put("swap-goals-different-folders")
    async swapGoalsDifferentFolders(@Req() req: AuthRequest, @Body() swapData: SwapGoalDifferentFolders){
