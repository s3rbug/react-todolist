...
const Folders = ({ match }: OwnProps) => {
    ...
    const setCurrentFolderById = useCallback(
        (id: number) => dispatch(setCurrentFolderByIdAction(id)),
        [dispatch]
    );
    ...
    const addGoal = useCallback(
        (text: string) => dispatch(addGoalAction(text)),
        [dispatch]
    );
    ...
    const setPageTitle = useCallback(
        (newTitle: string) => dispatch(setPageTitleAction(newTitle)),
        [dispatch]
    );
    useEffect(() => {
        if (
            ...
            setCurrentFolderById(parseInt(match.params.currentFolder));
    }, [match.params.currentFolder, setCurrentFolderById]);
        ...
        <div className={classes.root}>
            <Grid container className={classes.container} spacing={3}>
                {folders.map((folder: FolderType) => {
                    return (
                        <Grid key={folder.id} item xs={6} md={4} lg={3}>
                            <Folder
                                id={folder.id}
