...
const goalSlice = createSlice({
	...
	reducers: {
		...
		toggleChecked: (
			state,
			action: PayloadAction<{ folderId: string; goalId: string }>
		) => {
		...
		deleteGoal: (
			state,
			action: PayloadAction<{ goalId: string; folderId: string }>
		) => {
			...
			if (folderIndex !== -1) {
				state.folders[folderIndex].goals = state.folders[
					folderIndex
		...
		editFolderHeadline: (
			state,
			action: PayloadAction<{ folderId: string; headline: string }>
		) => {
		...
		addFolder: (state, action: PayloadAction<{ newFolder: FolderType }>) => {
			...
			if (state.currentFolders.includes(null)) {
				const index = state.currentFolders.findIndex(
		...
		swapGoalsDifferentFolders: (
			...
			const { fromFolderId, toFolderId, fromGoalIndex, toGoalIndex } =
				action.payload
			const fromFolderIndex = state.folders.findIndex(
				(folder) => folder.id === fromFolderId
			)
			const toFolderIndex = state.folders.findIndex(
				(folder) => folder.id === toFolderId
			)
			if (fromFolderIndex !== -1 && toFolderIndex !== -1) {
				const [removed] = state.folders[fromFolderIndex].goals.splice(
					fromGoalIndex,
					1
				)
		...
		swapGoalsSameFolder: (
			...
			if (folderIndex !== -1) {
				const [removed] = state.folders[folderIndex].goals.splice(
					fromGoalIndex,
					1
				)
		...
		reorderCurrentFolders: (
			state,
			action: PayloadAction<{ fromFolderId: string; toFolderId: string }>
		) => {
		...
		deleteFolder: (state, action: PayloadAction<{ folderId: string }>) => {
			...
			const hiddenFolders = state.folders.filter(
				(folder) => !state.currentFolders.includes(folder.id)
			)
			if (state.currentFolders.includes(folderId)) {
				const deletedFolderIndex = state.currentFolders.findIndex(
